...
- **☑️ Stay-open toggles for backdrop layers** — the 8 backdrop/Easter-egg toggles (Background Stars, Deep Space Gradient, Distant Galaxies, Nebula Clouds, Show Planets, Passing Comets, Screen-Dive Comet 🎯, Show Spacecraft) now **keep the menu open** when clicked, so you can flick multiple layers on and off in rapid succession and watch the scene rebuild live. Previously every click dismissed the menu and you had to reopen it to toggle the next setting. Implemented via a new reusable **`ToggleMenuItemView`** NSView (sister to the existing `SliderMenuView`) — assigning a custom view to `menuItem.view` bypasses macOS's automatic menu dismissal on click. Native NSButton checkbox styling with macOS menu font. New file: `src/ToggleMenuItemView.swift` (~42 lines). Click outside the menu or press Escape to dismiss as normal.
- **ℹ️ About HollywoodSaver… menu item** — new top-level menu entry above Contribute showing a native macOS dialog with app version, feature summary, credits, and license info. Includes the Ranger helmet as the dialog icon (pulled from `images/ranger.png` via the same `iconImagePath()` helper used by the menu bar). Three action buttons: **Close**, **Open GitHub Repo** (launches the repo in the default browser), and **View Full README** (opens the bundled `docs/ABOUT.md` file — tries the app bundle first, falls back to the on-disk path, then to the GitHub page). Properly activates `NSApp` so the dialog comes to the front from a menu bar app.
