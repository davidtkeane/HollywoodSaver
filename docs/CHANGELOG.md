...
### Added
- **⚡ Metal GPU Hyperspace Warp Engine** — brand new built-in GPU shader effect (`src/MetalWarpView.swift`)! Runs a real-time raymarched hyperspace tunnel shader entirely on the Apple Silicon GPU via Metal Shading Language (MSL) at 60/120 Hz ProMotion with **<1% CPU utilization**.
- **🌐 Interactive Web Wallpapers** — live HTML5 canvas & WebGL backgrounds via `WKWebView` (`src/WebWallpaperView.swift`)! Drop any `.html` / `.htm` file into the `web/` folder to run interactive live wallpapers and screensavers. Included sample wallpaper: `web/particles.html` (interactive cyber plexus canvas).
- **🖥️ Per-Monitor Media Assignment** — select and play different media or generative effects on each display independently (e.g. Video on Screen 1, Matrix Rain on Screen 2, Starfield on Screen 3) without interrupting playback on other screens. Includes dedicated **Displays** multi-monitor controller submenu with real-time status and single-display controls.
- **▶️ Sequential Video Playlist Mode** — auto-advances to the next video in `videos/` upon completion, with seamless folder loop-around. Added "Play All (Sequential)" menu item and "Sequential Playlist" toggle.
...
### Changed & Refactored
- **🏛️ Modularized `AppDelegate.swift`** — split monolithic 2,940-line controller into 8 domain-specific extension files:
  - `AppDelegate+Menu.swift` (1,033 lines) — Main status bar menu assembly and `NSMenuDelegate`.
  - `AppDelegate+Playback.swift` (556 lines) — Screensaver & ambient playback engine, rain effects, floating clock overlay.
  - `AppDelegate+Updates.swift` (383 lines) — GitHub release version checking, SHA-256 verification, auto-updater script.
  - `AppDelegate+BreakReminder.swift` (343 lines) — Break timer, Pomodoro engine, sound alerts, fullscreen break window, floating countdown HUD.
  - `AppDelegate+Settings.swift` (243 lines) — Settings actions, Matrix/Starfield/Photo config mutators, screen items submenu generator.
  - `AppDelegate+LockScreen.swift` (207 lines) — Lock screen window lifecycle, SHA-256 password setup/clear dialogs.
  - `AppDelegate+Sleep.swift` (152 lines) — Mac sleep (`IOPMSleepSystem`), sleep timers, floating sleep countdown HUD.
  - `AppDelegate.swift` (385 lines) — Core lifecycle, stored properties, path resolver, event observers, DRY helpers.
- **Codebase Expansion** — now 26 focused Swift source files in `src/` totaling 6,521 lines.

### Fixed & Optimized
- **LockScreen Shake Animation** — enabled layer backing on `passwordField` in `src/LockScreen.swift` for incorrect password shake cues.
- **Single-Photo Slideshow Loop** — fixed slideshow stall in `src/PhotoSlideshowView.swift` to continuously loop single-photo collections with randomized Ken Burns zoom/pan.
- **Dynamic Layout Resizing** — added `override func layout()` in `PhotoSlideshowView.swift` and `VideoPlayerView.swift` to sync layer frames to bounds.
- **Display Hotplugging & Parameter Observer** — added `NSApplication.didChangeScreenParametersNotification` listener to reposition overlays and playback windows when screens change.
- **Modernized Notifications** — upgraded `sendBreakNotification` from deprecated `NSUserNotificationCenter` to `UNUserNotificationCenter`.
- **Matrix Rain CPU & ProMotion Throttling** — throttled `setNeedsDisplay` to state ticks and preserved `CTLine` font fallback for Japanese Katakana glyphs.
- **Starfield Gradient Caching** — pre-cached static gradients and color spaces in `src/StarfieldWarpView.swift`.
- **GIF Autorelease Pool** — wrapped `CVDisplayLink` output callback in `@autoreleasepool { ... }` in `src/GifPlayerView.swift`.
- **InputMonitor Grace Period** — added 0.5s initial grace period and 20.0pt threshold to prevent accidental screensaver dismissals.
- **Translocation Cleanup** — cleaned up unused dynamic linker declarations in app translocation resolver.
